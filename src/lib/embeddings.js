...
async function getEmbeddingPipeline() {
  ...
  if (!embeddingPipeline) {
    try {
      ...
      // Use local_files_only: false to allow downloading models
      // This helps with serverless environments
      embeddingPipeline = await pipeline(
        'feature-extraction',
        'Xenova/distilbert-base-uncased',
        {
          // Try to use a more compatible configuration for serverless
          quantized: false, // Disable quantization which might require native libs
        }
      );
